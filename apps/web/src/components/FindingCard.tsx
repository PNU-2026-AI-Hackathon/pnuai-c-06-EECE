...
const LANES: {
  ...
  {
    // **「데이터시트가 아는 것」이라고 부르고 있었다.** 그런데 이 레인에는 두 종류가
    // 섞인다 — 제조사 PDF 에서 읽은 값과, 데이터시트가 없어서 **직접 재서** 넣은 값.
    //
    // 실제로 그래서 사용자가 데이터시트인 줄 알고 열었다가 남의 실측 기록을 읽는다.
    // 저가 모듈은 쓸 만한 데이터시트가 아예 없는 경우가 흔해서 앞으로도 계속 그렇다.
    //
    // 출처가 무엇인지는 각 사실에 붙은 인용 줄이 이미 정확히 말한다
    // (`Table5-1 · p.64` / `2026-08-21 · 자체 실측`). 레인 이름은 **둘을 다 덮는 말**이어야 한다.
    kind: "datasheet",
    label: "부품이 아는 것",
    unknown: "부품 목록(BOM)을 올리면 그 부품의 확인된 값을 찾아 이 줄을 채웁니다.",
    none: "이 부품에서 이 판정에 쓸 값을 찾지 못했습니다.",
  },
];
