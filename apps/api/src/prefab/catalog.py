...
CATALOG: tuple[RuleSpec, ...] = (
    ...
    RuleSpec(
        # **발견 루프가 찾아낸 첫 규칙이다.** 우리 규칙 13개가 이 모양을 하나도 안
        # 보고 있었고, 모델이 실측 보드 펌웨어에서 짚었다 (`discover/`).
        # 경고 등급인 이유는 규칙 파일에 적혀 있다 — 얼마나 나쁜지는 무엇이 달렸는지에
        # 달렸고, 치명으로 올리면 LED 하나 때문에 빨간불이 난다.
        "R16",
        "안전값을 쓰기 전에 출력으로 바꿔 부팅마다 순간 구동됨",
        "차별",
        Severity.WARNING,
        ("firmware",),
    ),
    RuleSpec(
        # **LLM 이 찾았고 코드가 확인했다.** 홀드아웃 보드에서 Sonnet 이 "U3 15번 핀이
        # 두 넷에 있다" 고 했고 넷리스트를 열어 보니 사실이었다. 우리 규칙 14개 중
        # 어느 것도 이 모양을 안 봤다.
        #
        # **회로도만으로 되니까 「기본」이다** (헌법 3절). 아쉽지만 그게 규칙이다 —
        # 펌웨어가 필요 없는 것을 차별로 올리면 우리가 뭘로 이기는지가 흐려진다.
        "R17",
        "한 핀이 서로 다른 두 네트에 연결됨",
        "기본",
        Severity.CRITICAL,
        ("netlist",),
    ),
