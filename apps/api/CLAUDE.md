...
### 알려진 버그
0. **회로도 넷리스트에서 핀맵이 안 풀린다 → R07·R08 이 조용하다.**
   `pinmap._match_part` 가 **X좌표 열 서명**으로 모듈을 알아보는데 회로도 넷리스트에는
   좌표가 없다. 우리 REV2 보드가 지금 그 상태다 (`board/board.net.xml`).
   R15 는 `_driven_nets` 가 핀 이름으로도 찾게 해 둬서 돈다 — **같은 길을 내주면 된다:
   회로도가 부품번호를 실어 오므로(`mpn.part_numbers`) 그 모듈의 실크 표로 이름을 푼다.**
   드리프트 데모를 만들다 발견했다 — 센서 핀을 옮겼는데 "변화 없음" 이 나왔다.
