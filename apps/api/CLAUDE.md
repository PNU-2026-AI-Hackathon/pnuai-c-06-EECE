...
> 이 문서는 **헌법**이다 — 무엇을 지켜야 하는지.
> **지금 어디까지 왔는지**는 [`HANDOFF.md`](../../HANDOFF.md) 에 있다.
...
- ~~인증 · 회원가입 · 결제 — 8/28까지 만들지 않는다~~
  **8/23, 팀장 결정으로 인증과 회원가입은 범위 안으로 들어왔다. 결제는 그대로 범위 밖이다.**

  들어올 때 지킬 것 세 가지를 같이 정한다. 이걸 안 지킬 거면 안 만드는 게 낫다.

  1. ~~**로그인은 더하는 것이지 막는 것이 아니다.**~~
     **8/24, 팀장 결정으로 검사에 로그인 벽을 세운다.** 정식 서비스로 나가면서
     사용자를 알아야 한다는 판단이다.

     **8/26, 벽의 위치를 옮겼다.** 결정 자체는 그대로다 — 계속 쓰려면 계정이 필요하다.
     바뀐 것은 **언제 요구하느냐**다.
     ...
     상태에서 계정을 내놓으라는 요구를 받고 있었다. 경쟁 서비스를 직접 써 보니
     그 벽이 아예 없는 곳도 있었다.

     게스트는 **한 번** 검사할 수 있다 (`web/guest.py`). 서명한 쿠키로 세고 DB 에
     안 넣는다. **요금 방어선이 아니다** — 쿠키를 지우면 다시 되고 그게 의도다.
     진짜 방어선은 요청 제한이다.

     **키를 들고 왔는데 안 맞으면 게스트로 흘려보내지 않는다.** CI 러너가 죽은
     키로 부르면 401 을 받아야 한다. 게스트로 통과시키면 검사는 성공하고 키가
     죽은 줄 아무도 모른다.

     **다만 결과 링크는 그대로 열어 둔다.** 링크를 받은 사람까지 가입시키면
     "링크 하나로 근거까지 보인다"는 우리 최대 강점이 사라지고, 요금표가 파는
     「결과 링크 공유」가 거짓말이 된다. 선은 여기다 —

         검사를 만드는 것   로그인 필요
         결과를 보는 것     주소를 알면 누구나 (주인이 있는 검사는 주인만)
  2. **계정이 사라질 수 있다는 사실을 조용히 두지 않는다.** 무료 플랜에는 영구
     디스크를 붙일 수 없어서 재배포마다 DB 가 날아간다. 추측하지 말고 **살아남았는지
     측정해서**(`web/storage.py`) 화면에 말한다. 확인 전에는 "안전하다"가 아니라
     "확인되지 않았다"이다 (헌법 2-2).
...
### 알려진 버그
-2. ~~**"디스크에 쓰지 않습니다"가 1MB 넘는 파일에서는 거짓이었다.**~~ **고쳤다 (8/23).**
   `/privacy` 에 그렇게 적어 뒀는데, starlette 의 `SpooledTemporaryFile` 은 기본값이
   1MB 라 그보다 큰 업로드를 **임시 파일로 디스크에 쓴다.** 우리 업로드 상한은 10MB 라
   1~10MB 구간이 통째로 그랬다. 코드를 안 보고 문구를 먼저 쓴 것이다 (헌법 2-4).

   추측이 아니라 `SpooledTemporaryFile.rollover` 를 가로채 세어서 확인했다 —
   0.5MB 는 0회, 2MB 는 1회. `spool_max_size` 를 상한 위로 올려 문구가 참이 되게 했고,
   `test_업로드는_디스크로_넘어가지_않는다` 가 같은 방식으로 이걸 지킨다.
   대신 메모리를 쓰므로 **본문 크기를 파서보다 먼저** 끊는다 (`web/app.py` 의 `guard`).

-1. ~~**검사 ID 를 훑으면 남의 회로도가 보인다.**~~ **고쳤다 (8/23).**
   `secrets.token_hex(3)` — 16진수 6자리, 1,670만 조합이었다.
   `GET /api/v1/checks/{id}` 에는 **인증이 없고**, 결과에는 사용자의 실제 소스 코드
   줄과 네트·부품 목록이 들어 있다. **초당 100회면 47시간에 전수 조사가 끝난다.**
   무료 플랜이라 요청 제한도 없었다. 16바이트(32자리)로 올렸다 —
   **ID 를 못 맞히는 것이 이 서비스 접근 통제의 전부다.**
...
| R17 | 한 핀이 서로 다른 두 네트에 연결됨 (단락) | 기본 | netlist | **동작** — **LLM 베이스라인이 찾고 코드가 확인한 규칙** |
...
**규칙이 어디서 왔는지가 이 표의 절반이다.**
    ...
    R17  **측정이 찾은 것 → 규칙**  LLM 대조 실험이 우리 사각지대를 드러냈다 `NEEDS` 어휘는 계약과 같은 `netlist` / `bom` / `firmware` 세 개뿐이다.
...
**`blocked(ctx) -> str | None` 은 선택이다** (8/24 추가). 입력은 다 있는데 규칙이
**스스로 못 돌리는** 경우를 말한다. 사유를 돌려주면 엔진이 `check()` 를 안 부르고
`skipped(precondition)` 으로 싣는다.

칩을 못 알아본 경우가 그렇다. 넷리스트도 펌웨어도 있는데 어느 칩인지 모르면
스트래핑 핀도 플래시 핀도 모른다. **그동안 그 규칙 5개가 조용히 빈 목록을 돌려줬고
화면에는 "규칙 14개 실행" 이라고 떴다** — 사용자는 다 검사해서 깨끗한 줄 읽는다.
남의 보드 22개에서 통째로 그랬다. 헌법 2-4 정면 위반이라 고쳤다.

**빈 목록과 못 돌린 것은 다르다.** 반환값이 `[]` 인 이유가 "봤는데 없다" 가 아니면
`blocked()` 로 올린다.
