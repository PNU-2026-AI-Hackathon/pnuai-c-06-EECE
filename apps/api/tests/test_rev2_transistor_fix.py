...
안된다" 고 보고했고, R15 가 그 네트를 짚었고, 2N3904 레벨시프트로 고쳤고,
**고친 보드가 도는 것까지 확인했다** — 사람이 앞에 있으면 켜지고 떨어지면 꺼진다.

그 영상이 가설 하나를 같이 지웠다. 처음엔 "mmWave 가 너무 민감해서 계속 감지 중"
일 수도 있었는데, **떨어질 때 꺼진다는 것은 센서가 제때 LOW 를 낸다는 뜻**이다.
문제는 센서가 아니라 이 인터페이스였다.
