...
> **확인 상태 (2026.08.17)**: 이 표는 좌표 + Seeed 공식 문서로 추론한 것이다.
> 하드웨어 담당(한지양)에게 물었고 **아직 확정을 못 받았다.** 답변 요지:
> "센서를 mmWave 로 바꾼 뒤 코딩을 안 해서 **아직 핀 넘버를 안 정했다**",
> "임의로 정해서 말할까?"
>
> **임의로 정하면 안 된다.** 핀 배정은 이미 PCB 라우팅으로 확정돼 있고,
> 이 `.d356` 이 그 증거다. 담당자가 "안 정했다"고 하는 건 *코드에서 쓸 핀*이고,
> 그건 이미 구리가 정해버렸다. 필요한 건 결정이 아니라 **PCB 를 열어서 읽는 것**이다.
>
> Flux 에서 `PRESENCE_3V3`(mmWave OUT)과 `_IN_ACTIVE_LOW`(릴레이)가
> XIAO 의 어느 실크 라벨에 붙었는지만 확인하면 이 표가 확정된다.
