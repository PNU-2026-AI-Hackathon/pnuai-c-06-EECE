...
export function MockMissedPanel() {
  ...
  const totalLoss = mockMissedOpportunities.reduce((s, o) => s + o.estimatedLoss, 0);
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex flex-wrap gap-8">
            ...
            <div>
              <p className="text-sm text-muted-foreground">추정 손실</p>
              <p className="text-2xl font-bold">{formatWon(m.estimatedLoss)}</p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground">목록 전체 손실 합</p>
              <p className="text-2xl font-bold">{formatWon(totalLoss)}</p>
            </div>
          </div>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right">추정 손실</TableHead>
            ...
            <TableBody>
              {mockMissedOpportunities.map((o) => (
                <TableRow key={o.id}>
                  ...
                  <TableCell className="text-right">{formatWon(o.estimatedLoss)}</TableCell>
